...
def parse_now_playing_element(root: ET.Element) -> dict[str, Any]:
    ...
    play_state = play_status_elem.text if play_status_elem is not None else None
    if not play_state:
        play_state = "UNKNOWN"
...
def get_now_playing(ip: str) -> dict[str, Any]:
    ...
    try:
        response = session.get(url, timeout=HTTP_GET_TIMEOUT_SECONDS)
...
def get_volume(ip: str) -> int | None:
    """
    ...
    try:
        ...
        if actual_volume_elem is not None and actual_volume_elem.text is not None:
            return int(actual_volume_elem.text)
            
        return None
    except (RequestException, ET.ParseError, TypeError, ValueError) as e:
